...
export default function VirtualFormStudent() {
  ...
  function getStudentResponse(e: ChangeEvent<HTMLInputElement>) {
  ...
  return (
    <div className={styles.divContain}>
      <Header2
        ambientName={ambientData.length == 0 ? "Carregando..." : ambientData}
        url="virtualAmbientStudent"
      />
      <main className={styles.main}>
        <form className={styles.virtualFormStudent} onSubmit={handleSubmit}>
          <RangeInput
            id="degreeUnderstanding"
            label="Indique o seu grau de compreensão da disciplina:"
            onChange={getStudentResponse}
          />
          <RangeInput
            id="degreeInterest"
            label="Indique o seu grau de interesse da disciplina:"
            onChange={getStudentResponse}
          />
          <RangeInput
            id="levelDifficulty"
            label="Indique o seu grau de confiança sobre a disciplina:"
            onChange={getStudentResponse}
          />
          <RangeInput
            id="degreeEmocional"
            label="Indique o seu estado emocional durante a aula:"
            onChange={getStudentResponse}
          />
          <RangeInput
            id="degreeSatisfaction"
            label="Indique o sua satisfação sobre a aula:"
            onChange={getStudentResponse}
          />
          <RadioInput
            id="topicDifficulty"
            label="Tópico de dificuldade"
            options={optionsDifficulty}
            name="topicDifficulty"
            onChange={getStudentResponse}
          />
          <RadioInput
            id="reinforcementNeed"
            label="Necessidade de Reforço"
            options={optionsReinforcementNeed}
            name="reinforcementNeed"
            onChange={getStudentResponse}
          />
          <RadioInput
            id="timeDedicatedStudy"
            label="Tempo dedicado para estudo(mínimo):"
            options={optionsTimeDedicatedStudy}
            name="timeDedicatedStudy"
            onChange={getStudentResponse}
          />
